...
///////////////////////////////////////////////////////////////////////////////////////////////
// create a matrix whose entries are sin(a_{ij})
// (component-wise sin value of my data)
int Matrix::Sin() {
  for (size_t j=0; j<ncols; j++)
    for (size_t i=0; i<nrows; i++)
...
// create a matrix whose entries are cos(a_{ij})
// (component-wise cos value of my data)
int Matrix::Cos() {
  for (size_t j=0; j<ncols; j++)
    for (size_t i=0; i<nrows; i++)
      data[j][i] = cos(data[j][i]);
  return 0;
}
...
Matrix Random(size_t m, size_t n) {
  ...
  for (size_t j=0; j<n; j++)
    for (size_t i=0; i<m; i++)
      C.data[j][i] = dist(engine); // Generate pseudo-random number.
  return C;
}*/
...
////////////////////////////////////////////////////////////////////////////////
// computes the sin of the entries of a matrix A
Matrix Sine(const Matrix& A) {
  ...
  // call existing Inverse routine for computations
  if (X.Sin() != 0)
    cerr << "Sine: error computing sine matrix\n";

  // return result
  return X;
}

// computes the cos of the entries of a matrix A
Matrix Cosine(const Matrix& A) {
...
// Adds up entries of columns of matrix A
Matrix Sum(const Matrix& A) {
  Matrix C(1,A.Cols());
  for (size_t j=0; j<A.Cols(); j++)
    for (size_t i=0; i<A.Rows(); i++)
      C.data[j][0] += A.data[j][i];
...
//--- supplementary ma`trix-vector arithmetic routines ---
